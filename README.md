...
- 源码统一使用 UTF-8 编码，面向项目维护者的注释和文档字符串使用简体中文。
- Python 模块应在文件顶部说明职责；公共类、函数和复杂流程按需补充文档字符串。
- 注释重点解释业务意图、数据流、边界条件和实现原因，不逐行复述代码语法。
- Vue、JavaScript 与 CSS 按功能区块添加说明；流式协议、响应式布局等非直观逻辑应明确记录约束。
- 修改实现时同步更新相邻注释，避免注释与实际行为不一致。
